...
class AssignWorkerView(LoginRequiredMixin, AgencyManagerRequiredMixin, FeatureRequiredMixin, View):
    ...
    def post(self, request, shift_id, *args, **kwargs):
        ...
        if shift.is_full:
            if is_ajax:
                return ajax_response_with_message(
                    False, "Cannot assign worker. The shift is already full."
                )
            else:
        ...
        if ShiftAssignment.objects.filter(shift=shift, worker=worker).exists():
            if is_ajax:
                return ajax_response_with_message(
                    False, f"Worker {worker.get_full_name()} is already assigned to this shift."
                )
            else:
                messages.error(
                    request, f"Worker {worker.get_full_name()} is already assigned to this shift."
                )
                return redirect("shifts:shift_detail", pk=shift.id)
        ...
        try:
            ...
            logger.exception(
                f"Unexpected error when assigning worker {worker.id} to shift {shift.id}: {e}"
            )
...
class UnassignWorkerView(
    ...
    def get(self, request, shift_id, assignment_id, *args, **kwargs):
        ...
        return render(
            request,
            "shifts/unassign_worker_confirm.html",
