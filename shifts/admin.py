...
class ShiftCapacityFilter(SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        if self.value() == "full":
            return queryset.annotate(assignments_count=Count("assignments")).filter(
                assignments_count__gte=F("capacity")
            )
        elif self.value() == "available":
            return queryset.annotate(assignments_count=Count("assignments")).filter(
                assignments_count__lt=F("capacity")
            )
        return queryset
...
class ShiftAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("name", "shift_code", "shift_date", "end_date")}),
        ("Timing", {"fields": ("start_time", "end_time", "is_overnight", "duration")}),
        ("Capacity & Role", {"fields": ("capacity", "shift_type", "shift_role")}),
        ("Agency & Status", {"fields": ("agency", "status", "is_active", "is_full")}),
        ("Financials", {"fields": ("hourly_rate", "total_pay", "total_hours")}),
        ("Notes", {"fields": ("notes",)}),
        (
            "Location",
            {
        ...
        ("Completion", {"fields": ("is_completed", "completion_time", "signature")}),
    )
...
class ShiftAssignmentAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("worker", "shift", "role", "status", "attendance_status")}),
        (
            "Completion Details",
            {
                "fields": (
