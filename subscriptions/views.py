                ...
                if should_sync and agency.stripe_customer_id:
                    try:
                        stripe_subscriptions = stripe.Subscription.list(
                            customer=agency.stripe_customer_id, status="active", limit=1
                        )
                        ...
                        if stripe_subscriptions.data:
                            ...
                            if (
                                ...
                                subscription.save()
                                logger.info(
                                    f"Synced subscription data from Stripe for {agency.name}"
                                )
                ...
                if subscriptions.data:
                    ...
                    try:
                        ...
                        try:
                            ...
                            logger.info(
                                f"Created new subscription in success page for {agency.name}"
                            )
    ...
    def handle_checkout_session_completed(self, session):
        ...
        try:
            ...
            if (
                ...
                and existing_subscription.stripe_subscription_id != subscription_id
            ):
                try:
                    ...
                    logger.info(
                        f"Old subscription {existing_subscription.stripe_subscription_id} no longer exists in Stripe."
                    )
            ...
            if subscription.get("items", {}).get("data"):
                ...
                try:
                    ...
                    if existing_subscription.stripe_subscription_id == stripe_subscription_id:
                        ...
                        logger.info(
                            f"Updated existing subscription {stripe_subscription_id} for agency {agency.name}"
                        )
                    else:
                        logger.warning(
                    ...
                    logger.info(
                        f"Created new subscription {stripe_subscription_id} for agency {agency.name}"
                    )
            else:
...
class SubscriptionChangeView(LoginRequiredMixin, AgencyOwnerRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            # Sort plans by price
            same_cycle_plans = sorted(
                same_cycle_plans, key=lambda p: p.price, reverse=(self.change_type == "downgrade")
            )
            ...
            if checkout_session.status == "complete":
                ...
                if is_cycle_change:
                    try:
                        ...
                        logger.info(
                            f"Cancelled old subscription: {old_subscription_id} after billing cycle change"
                        )
                    except stripe.error.StripeError as e:
                ...
                try:
                    ...
                    if plan_id:
                        ...
                        logger.error(
                            f"Could not find plan for Stripe price ID: {plan_id_from_stripe}"
                        )
                        messages.warning(
                            request,
                            "Your subscription was changed but we couldn't identify the plan. Please contact support.",
                        )
                        return render(request, "subscriptions/success.html")
                ...
                messages.warning(
                    request,
                    "Your subscription change was not completed. Please try again or contact support.",
                )
            ...
            try:
                    ...
                    if subscriptions.data:
                        ...
                        try:
                            invoices = stripe.Invoice.list(
                                customer=agency.stripe_customer_id, limit=1
                            )
                            if invoices.data:
                ...
                if subscription and subscription.is_active:
                    ...
                    alternate_cycle_plans = Plan.objects.filter(
                        name=current_plan.name, billing_cycle=alternate_cycle, is_active=True
                    ).order_by("price")
