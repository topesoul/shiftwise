            ...
            if not subscriptions.exists():
                ...
                for idx, sub in enumerate(subscriptions, 1):
                    ...
                    if sub.current_period_end and sub.current_period_end < now and sub.is_active:
                        self.stdout.write(self.style.ERROR("  Subscription period has ended but is still marked active"))
                        if fix_mode:
                            sub.is_active = False
                            sub.is_expired = True
                            sub.status = "canceled"
                            sub.save()
                            self.stdout.write(self.style.SUCCESS("  Fixed: Marked as inactive and expired"))
                    
                    # Verify Stripe subscription if ID exists
                    if sub.stripe_subscription_id and customer_valid:
                        try:
                            stripe_sub = stripe.Subscription.retrieve(sub.stripe_subscription_id)
                            self.stdout.write(self.style.SUCCESS(f"  Stripe subscription exists: {stripe_sub.status}"))
                            
                            # Check for status mismatch
                            stripe_active = stripe_sub.status == "active"
                            if sub.is_active != stripe_active:
                                self.stdout.write(self.style.ERROR(f"  Status mismatch: Local={sub.is_active}, Stripe={stripe_active}"))
                                if fix_mode:
                                    sub.is_active = stripe_active
                                    sub.status = stripe_sub.status
                                    sub.save()
                                    self.stdout.write(self.style.SUCCESS(f"  Fixed: Updated status to match Stripe"))
                            
                        except stripe.error.InvalidRequestError:
                            self.stdout.write(self.style.ERROR("  Stripe subscription ID is invalid"))
                            ...
                            self.stdout.write(self.style.ERROR(f"  Error retrieving Stripe subscription: {e}"))
        ...
        if customer_valid:
            ...
            try:
                ...
                if not stripe_subs.data:
                    ...
                    local_sub_ids = set(subscriptions.values_list('stripe_subscription_id', flat=True))
                            ...
                            if fix_mode:
                                try:
                                    ...
                                    try:
                                        ...
                                        self.stdout.write(self.style.ERROR(f"  Plan with price ID {price_id} not found"))
                                        continue
                                    ...
                                    with transaction.atomic():
                                        ...
                                        new_sub.save()
                                        self.stdout.write(self.style.SUCCESS(f"  Fixed: Created local subscription record"))
                                except Exception as e:
                                    self.stdout.write(self.style.ERROR(f"  Error creating local subscription: {e}"))
