        ...
        for agency in agencies:
            try:
                ...
                stripe_subs = stripe.Subscription.list(
                    customer=agency.stripe_customer_id, status="active", limit=5
                )

                if stripe_subs.data:
                    # Sort by created date (newest first)
                    stripe_subs_sorted = sorted(
                        stripe_subs.data, key=lambda x: x.get("created", 0), reverse=True
                    )
                ...
                self.stdout.write(
                    self.style.ERROR(f"Error syncing subscriptions for {agency.name}: {e}")
                )
                error_count += 1
