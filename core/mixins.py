...
def handle_permission_error(request, message, redirect_url="home:home"):
    ...
    Detects session timeouts and redirects to login with appropriate message.
    """
    if "you do not have permission" in message.lower() and request.user.is_anonymous and "login" not in redirect_url:
        messages.info(
            request, 
